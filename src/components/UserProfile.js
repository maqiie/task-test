...
// const apiBaseUrl = "https://task-test-backend.onrender.com";
...
//       ? `https://task-test-backend.onrender.com${userData.image_url}`
//       : defaultImage,
//   });
...
const UserProfile = ({ userData, onLogout }) => {
  const [formData, setFormData] = useState({
    ...
    uid: userData?.uid || "",
    image: null,
    image_url: userData?.image_url || defaultImage,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.patch("/auth", formDataWithImage, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          "Content-Type": "multipart/form-data",
        },
